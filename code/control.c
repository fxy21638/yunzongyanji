// ====================================================================
// control.c — 49.9 舵机 PD + 差速电机 + 赛车线 + 编码器速度 PID
// 引脚定义保持不变
// ====================================================================
...
/* ================================================================
 * 第 2 节 — 赛车线偏移 (49.9 Racing_Line_Offset)
 * ================================================================ */

#define RACING_LINE_START_ERR 0//9
...
/* ================================================================
 * 第 3 节 — 舵机 PD 参数选档 (49.9 servo_pd_set)
 * ================================================================ */

static float g_z_p = 1.20f, g_z_d = 3.60f;

static void servo_pd_set(void)
{
    int16_t abs_err = error;
    if (abs_err < 0)
        abs_err = -abs_err;

    //    if (huandao_state != HUANDAO_NONE)
    //    {
    //        if (huandao_state == HUANDAO_STATE6 || huandao_state == HUANDAO_STATE7)
    //        {
    //            g_z_p = 1.70f;
    //            g_z_d = 4.90f;
    //        }
...
/* ================================================================
 * 第 4 节 — 舵机 PD 计算+输出 (49.9 server_PD + servo_Control)
 * ================================================================ */

#define SERVO_PWM_LIMIT 50
...
/* ================================================================
 * 第 5 节 — 编码器 EMA 滤波 (49.9 EncoderCount)
 * ================================================================ */
...
/* ================================================================
 * 第 6 节 — 增量 PID + 低通 D (49.9 PID_Increase)
 * ================================================================ */
                          ...
                          float *d_filt, float target, float actual)
{
    ...
    *e1 = *e;
    *e = err;
    *out += MOTOR_KP * p_term + MOTOR_KI * i_term + MOTOR_KD * (*d_filt);
    if (*out > (float)PID_LIMIT)
        *out = (float)PID_LIMIT;
    if (*out < -(float)PID_LIMIT)
        *out = -(float)PID_LIMIT;
    return *out;
...
/* ================================================================
 * 第 7 节 — 速度斜坡 (49.9 Motor_Target_Ramp)
 * ================================================================ */
...
/* ================================================================
 * 第 9 节 — 差速 (49.9 Final_Motor_Control)
 * ================================================================ */
...
/* ================================================================
 * 第 10 节 — 速度档位 (49.9 Track_Motor_Control)
 * ================================================================ */
...
//    {
//        if (huandao_state == HUANDAO_STATE6 || huandao_state == HUANDAO_STATE7)
//            final_motor_control(22, 0.30f, 10);
//        else
...
/* ================================================================
 * 第 11 节 — 5ms 定时器 ISR (49.9 pit_handler 核心逻辑)
 * ================================================================ */
