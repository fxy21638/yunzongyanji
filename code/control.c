...
static void servo_pd(void)
{
    ...
    servo_d_filter = SERVO_D_FILTER * servo_d_filter
                   + (1.0f - SERVO_D_FILTER) * d_error;
    servo_out = (float)error * g_z_p + servo_d_filter * g_z_d;
    if (servo_out > SERVO_LIMIT)  servo_out = SERVO_LIMIT;
    if (servo_out < -SERVO_LIMIT) servo_out = -SERVO_LIMIT;
                          ...
                          float *d_filt, float target, float actual)
{
    ...
    *e  = err;
    ...
    if (i_contrib > MOTOR_I_MAX)  i_contrib = MOTOR_I_MAX;
    if (i_contrib < -MOTOR_I_MAX) i_contrib = -MOTOR_I_MAX;
...
{
    if (target > now + MOTOR_RAMP_UP)  return now + MOTOR_RAMP_UP;
    if (target < now - MOTOR_RAMP_DN)  return now - MOTOR_RAMP_DN;
    return target;
}
...
static void final_motor_control(int16_t base_speed, float diff_k, int16_t diff_limit)
{
    ...
    if (target_l < MOTOR_MIN_SPEED) target_l = MOTOR_MIN_SPEED;
    if (target_r < MOTOR_MIN_SPEED) target_r = MOTOR_MIN_SPEED;
...
static void track_motor_control(void)
{
    ...
    else
    {
        ...
        if (spd < MOTOR_MIN_SPEED) spd = MOTOR_MIN_SPEED;
        dk = 0.10f + (float)a * 0.003f;
        if (dk > DIFF_K_MAX) dk = DIFF_K_MAX;
        dl = (int16_t)(10 + (a / 3));
        if (dl > 15) dl = 15;
...
static void control_timer_callback(void)
{
...
#if SLOPE_ENABLE
    {
        ...
        if(roll>SLOPE_ENTER_DEG||roll<-SLOPE_ENTER_DEG){slope_cnt++;if(slope_cnt>3)g_slope_detected=1;}
        else if(roll<SLOPE_EXIT_DEG&&roll>-SLOPE_EXIT_DEG){slope_cnt=0;g_slope_detected=0;}
        else{slope_cnt=0;g_slope_detected=0;}
    }
#endif
